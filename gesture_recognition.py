...
def is_thumbs_up(landmarks):
    ...
    thumb_extended = (
        ...
        abs(thumb_tip.x - wrist.x) > 0.1  # Kciuk znacznie oddalony od nadgarstka
    )
 
    # 2. Dodatkowe sprawdzenie dla pozostałych palców: ich końcówki są blisko nadgarstka
    fingers_close_to_wrist = (
        abs(index_tip.x - wrist.x) < 0.1 and
        abs(middle_tip.x - wrist.x) < 0.1 and
        abs(ring_tip.x - wrist.x) < 0.1 and
        abs(pinky_tip.x - wrist.x) < 0.1
    )
 
    return thumb_extended and fingers_close_to_wrist 
